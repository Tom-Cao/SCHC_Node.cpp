...
SCHC_Node_Ack_on_error::SCHC_Node_Ack_on_error()
{
}
...
uint8_t SCHC_Node_Ack_on_error::init(uint8_t ruleID, uint8_t dTag, uint8_t windowSize, uint8_t tileSize, uint8_t n, uint8_t ackMode, SCHC_Node_Stack_L2 *stack_ptr, int retTimer, uint8_t ackReqAttempts)
{
#ifdef MYTRACE
    Serial.println("SCHC_Node_Ack_on_error::init - Entering the function");
#endif
...
#ifdef MYTRACE
    Serial.println("SCHC_Node_Ack_on_error::init - Leaving the function");
#endif

    return 0;
}

uint8_t SCHC_Node_Ack_on_error::start(char *msg, int len)
{
    this->TX_INIT_send_fragments(msg, len);
    return 0;
}

uint8_t SCHC_Node_Ack_on_error::enqueue_message(char* msg, int len)
{
    std::tuple<char*, int> myTuple(msg, len);
    _queue.push_back(myTuple);

#ifdef MYTRACE
        Serial.println("SCHC_Node_Ack_on_error::queue_message - Data successfully queue");
#endif

    return 0;
}

uint8_t SCHC_Node_Ack_on_error::execute(char *msg, int len)
{
#ifdef MYTRACE
    Serial.println("==================================================");
    Serial.println("SCHC_Node_Ack_on_error::execute - Entering the function");
#endif
    if(msg!=NULL)
    {
        ...
        {
            #ifdef MYTRACE
            Serial.println("SCHC_Node_Ack_on_error::execute - Calling to TX_INIT_send_fragments()");
            #endif
        ...
        {
            #ifdef MYTRACE
            Serial.println("SCHC_Node_Ack_on_error::execute - Calling to TX_WAIT_x_ACK_receive_ack()");
            #endif
        ...
        {
            #ifdef MYTRACE
            Serial.println("SCHC_Node_Ack_on_error::execute - Calling to TX_RESEND_MISSING_FRAG_send_fragments()");
            #endif
        ...
        {
            #ifdef MYINFO
            Serial.println("SCHC_Node_Ack_on_error::execute - Warning!!, there is no assigned method to process the message");
            #endif
        }
    }
    else
    {
        ...
        {
            #ifdef MYTRACE
            Serial.println("SCHC_Node_Ack_on_error::execute - Calling to TX_SEND_send_fragments()");
            #endif
        ...
        {
            #ifdef MYTRACE
            Serial.println("SCHC_Node_Ack_on_error::execute - Calling to TX_END_free_resources()");
            #endif
        ...
        {
            #ifdef MYTRACE
            Serial.println("SCHC_Node_Ack_on_error::execute - Do nothing!!");
            #endif
        }
        else
        {
...
#ifdef MYTRACE
    Serial.println("SCHC_Node_Ack_on_error::execute - Leaving the function");
    Serial.println("=================================================");
#endif
    return 0;
}

void SCHC_Node_Ack_on_error::message_reception_loop()
{
#ifdef MYTRACE
    Serial.println("SCHC_Node_Ack_on_error::message_reception_loop - Starting message reception loop.");
#endif
    ...
    while (_running)
    {
        ...
        {
            #ifdef MYTRACE
            Serial.println("SCHC_Node_Ack_on_error::message_reception_loop - Extracting message from the queue.");
            #endif
...
#ifdef MYDEBUG
            Serial.println("SCHC_Node_Ack_on_error::message_reception_loop - Leaving message reception loop.");
#endif
    
}

uint8_t SCHC_Node_Ack_on_error::TX_INIT_send_fragments(char *msg, int len)
{
#ifdef MYTRACE
    Serial.println("SCHC_Node_Ack_on_error::TX_INIT_send_fragments - Entering the function");
#endif
    ...
    if(res==1)
    {
        Serial.println("SCHC_Node_Ack_on_error::TX_INIT_send_fragments - ERROR when splitting the message into tiles");
    }


    /* memory allocated for pointers of each bitmap. */
    _bitmapArray = new uint8_t*[_nWindows];      // * Liberada en SCHC_Node_Ack_on_error::destroy_machine()

    /* memory allocated for the 1s and 0s for each bitmap. */ 
    for(int i = 0 ; i < _nWindows ; i++ )
    {
        _bitmapArray[i] = new uint8_t[_windowSize]; // * Liberada en SCHC_Node_Ack_on_error::destroy_machine()
    }
...
#ifdef MYTRACE
    Serial.println("SCHC_Node_Ack_on_error::TX_INIT_send_fragments - Leaving the function");
#endif
    return res;
}

uint8_t SCHC_Node_Ack_on_error::TX_SEND_send_fragments()
{
#ifdef MYTRACE
    Serial.println("SCHC_Node_Ack_on_error::TX_SEND_send_fragment - Entering the function");
#endif
    ...
    SCHC_Node_Message encoder;           // encoder 
    ...
    if(_ackMode==ACK_MODE_ACK_END_WIN)
    {
        if(_send_schc_ack_req_flag == true)
        {
            ...
            if(res==1)
            {
                Serial.println("SCHC_Node_Ack_on_error::TX_WAIT_x_ACK_receive_ack - ERROR sending L2 frame");
                return 1;
            }
            ...
            #ifdef MYTRACE
            Serial.println("SCHC_Node_Ack_on_error::TX_SEND_send_fragment - Leaving the function");
            #endif
        ...
        if(n_remaining_tiles>payload_available_in_tiles)
        {
            ...
            */
            ...
            if(res==1)
            {
                Serial.println("SCHC_Node_Ack_on_error::sendRegularFragment - ERROR sending L2 frame");
                return 1;
            }
...
uint8_t SCHC_Node_Ack_on_error::TX_WAIT_x_ACK_receive_ack(char *msg, int len)
{
#ifdef MYTRACE
    Serial.println("SCHC_Node_Ack_on_error::TX_WAIT_x_ACK_receive_ack - Entering the function");
#endif
    
    SCHC_Node_Message decoder;
    uint8_t msg_type = decoder.get_msg_type(SCHC_FRAG_LORAWAN, SCHC_FRAG_UPDIR_RULE_ID, msg, len);

    if(msg_type == SCHC_ACK_MSG)
    {
        #ifdef MYTRACE
        Serial.println("SCHC_Node_Ack_on_error::TX_WAIT_x_ACK_receive_ack - Receiving a SCHC ACK msg");
        #endif
        _retrans_ack_req_flag = false;
    ...
    {
        #ifdef MYTRACE
        Serial.println("SCHC_Node_Ack_on_error::TX_WAIT_x_ACK_receive_ack - Receiving a SCHC Compound ACK msg");
        #endif
        _retrans_ack_req_flag = false;
...
#ifdef MYTRACE
    Serial.println("SCHC_Node_Ack_on_error::TX_WAIT_x_ACK_receive_ack - Leaving the function");
#endif 
    return 0;
}

uint8_t SCHC_Node_Ack_on_error::TX_END_free_resources()
{
#ifdef MYTRACE
    Serial.println("SCHC_Node_Ack_on_error::TX_END_free_resources - Entering the function");
#endif

#ifdef MYINFO
    Serial.println("SCHC_Node_Ack_on_error::TX_END_free_resources - Releasing resources and finish machines and session");
#endif
...
uint8_t SCHC_Node_Ack_on_error::TX_RESEND_MISSING_FRAG_send_fragments(char *msg, int len)
{
#ifdef MYTRACE
    Serial.println("SCHC_Node_Ack_on_error::TX_RESEND_MISSING_FRAG_send_fragments - Entering the function");
#endif
    ...
    else if(_ackMode==ACK_MODE_ACK_END_SES)
    {
        if(_send_schc_ack_req_flag == true)
        {
            ...
            {
                /* Sending a SCHC All-1 fragment */
                SCHC_Node_Message    encoder_all_1;
                int payload_len             = _lastTileSize;                    // tamaño del last tile en bytes
                char* schc_all_1_message    = new char[payload_len + 1 + 4];        // liberado en linea 285. buffer para el SCHC message: header (1B) + rcs (4B) + payload
            ...
            /* Se envía un SCHC ACK REQ para empujar el envio en el downlink
            del SCHC ACK enviado por el SCHC Gateway */
            SCHC_Node_Message encoder_ack_req;
            char* schc_ack_req_msg      = new char[1];      // liberado en linea 308
            int schc_ack_req_msg_len    = encoder_ack_req.create_ack_request(_ruleID, 0, _last_confirmed_window, schc_ack_req_msg);

            /* Imprime los mensajes para visualizacion ordenada */
            encoder_ack_req.print_msg(SCHC_ACK_REQ_MSG, schc_ack_req_msg, schc_ack_req_msg_len);
            ...
            #ifdef MYTRACE
            Serial.println("SCHC_Node_Ack_on_error::TX_RESEND_MISSING_FRAG_send_fragments - Leaving the function");
            #endif
...
uint8_t SCHC_Node_Ack_on_error::mtuUpgrade(int mtu)
{
#ifdef MYTRACE
    Serial.println("SCHC_Node_Ack_on_error::mtuUpgrade - Entering the function");
#endif

#ifdef MYINFO
    Serial.print("SCHC_Node_Ack_on_error::mtuUpgrade - updating the MTU to: ");
    Serial.print(mtu);
...
#ifdef MYTRACE
    Serial.println("SCHC_Node_Ack_on_error::mtuUpgrade - Leaving the function");
#endif

    return 0;
}

uint8_t SCHC_Node_Ack_on_error::divideInTiles(char *buffer, int len)
{
...
#ifdef MYTRACE
    Serial.println("SCHC_Node_Ack_on_error::divideInTiles - Entering the function");
#endif
...
*/
...
#ifdef MYINFO
    Serial.print("SCHC_Node_Ack_on_error::divideInTiles - Full Tiles number: ");
    Serial.print(_nFullTiles);
    Serial.println(" tiles");
    Serial.print("SCHC_Node_Ack_on_error::divideInTiles - Last Tile Size: ");
    Serial.print(_lastTileSize);
    Serial.println(" bytes");
#endif 

    // memory allocated for elements of rows.
    _tilesArray = new char*[_nFullTiles];           // Liberado en metodo SCHC_Node_Ack_on_error::TX_END_free_resources()

    // memory allocated for  elements of each column.  
    for(int i = 0 ; i < _nFullTiles ; i++ )
    {
        _tilesArray[i] = new char[_tileSize];       // Liberado en metodo SCHC_Node_Ack_on_error::TX_END_free_resources()
    }
    ...
    _lastTile = new char[_lastTileSize];            // Liberado en metodo SCHC_Node_Ack_on_error::TX_END_free_resources()
...
#ifdef MYINFO
    Serial.print("SCHC_Node_Ack_on_error::divideInTiles - number of SCHC Windows: ");
...
#ifdef MYINFO
    Serial.print("SCHC_Node_Ack_on_error::divideInTiles - RCS before to send: ");
    Serial.println(_rcs);
#endif


#ifdef MYTRACE
    Serial.println("SCHC_Node_Ack_on_error::divideInTiles - Leaving the function");
#endif

    return 0;
}

uint8_t SCHC_Node_Ack_on_error::extractTiles(uint8_t firstTileID, uint8_t nTiles, char *buff)
{
#ifdef MYTRACE
    Serial.println("SCHC_Node_Ack_on_error::extractTiles - Entering the function");
#endif
...
#ifdef MYTRACE
    Serial.println("SCHC_Node_Ack_on_error::extractTiles - Leaving the function");
#endif

    return 0;
}

int SCHC_Node_Ack_on_error::getCurrentTile_ptr(int window, int bitmap_ptr)
{
    return (window * _windowSize) + bitmap_ptr;
}

uint8_t SCHC_Node_Ack_on_error::get_current_fcn(int bitmap_ptr)
{
    return (_windowSize - 1) - bitmap_ptr;
}

void SCHC_Node_Ack_on_error::printTileArray()
{
    Serial.println("***********************************************");
    for(int i=0; i<_nFullTiles; i++)
    {
...
uint32_t SCHC_Node_Ack_on_error::calculate_crc32(const char *data, size_t length) 
{
...
void SCHC_Node_Ack_on_error::send_ack_req()
{
#ifdef MYTRACE
    Serial.println("SCHC_Node_Ack_on_error::send_ack_req - Sending SCHC ACK Req");
#endif    
    SCHC_Node_Message encoder;
...
#ifdef MYTRACE
    Serial.println("SCHC_Node_Ack_on_error::send_ack_req - Leaving Function");
#endif 
}

void SCHC_Node_Ack_on_error::send_sender_abort()
{
#ifdef MYDEBUG
    Serial.println("SCHC_Node_Ack_on_error::send_sender_abort - Sending SCHC Sender-Abort");
#endif  
}
